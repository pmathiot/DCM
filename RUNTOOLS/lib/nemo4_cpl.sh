...
CN_DIRELM=${CONFIG_CASE}-XIOS       # receive elmer output files
...
echo ''
echo " [1.3] define run start/end/length "
echo ' ================================='
echo ''
...
echo "           <CN_DIAOBS> to $DDIR/${CN_DIAOBS}.$no"
echo ""
sed -e "s/<NN_NO>/$no/" \
...
if [ $DIAOBS = 1 ] ; then  # modify namelist block namobs (and get data files)
    ...
    ENACT=0  
    tmp=$(LookInNamelist ln_ena  namelist) ; tmp=$(normalize $tmp)
    if [ $tmp = T ] ; then ENACT=1 ; fi

    SLA=0
    tmp=$(LookInNamelist ln_sla  namelist) ; tmp=$(normalize $tmp)
...
echo ""
echo "    [2.1.4] cp namelist ocean to NEMO namelist ocean name (cfg and ref)"
echo ""
...
echo ""
echo "    [2.1.4] check/build directory tree"
echo ""
...
ZDFIWM=0
tmp=$( LookInNamelist ln_zdfiwm  namelist ) ; tmp=$( normalize $tmp ) 
...
# tracer damping 
TRADMP=0 
tmp=$(LookInNamelist ln_tradmp  namelist) ; tmp=$(normalize $tmp )
...
if [ $FLOAT = 1 ] ; then
    tmp=$(LookInNamelist ln_rstflo  namelist) ; tmp=$(normalize $tmp)
...
# Geothermal heating
GEOTH=0
tmp=$(LookInNamelist ln_trabbc  namelist) ; tmp=$(normalize $tmp)
if [ $tmp = T ] ; then 
    GEOTH=1
    nn_geoflx=$(LookInNamelist nn_geoflx  namelist)
...
# Iceberg calving
ICB=0
tmp=$(LookInNamelist ln_icebergs  namelist) ; tmp=$(normalize $tmp)
if [ $tmp = T ] ; then
    ICB=1
    if [ $ENSEMBLE = 1 ] ; then 
...
TRACOEF2D=0 ; TRACOEF3D=0
   tmp=$(LookInNamelist ln_traldf_OFF namelist) ; tmp=$(normalize $tmp)
   if [ $tmp = F ] ; then   #  Diffusion is ON !
     nn_aht_ijk_t=$(LookInNamelist nn_aht_ijk_t namelist)
     if [ $nn_aht_ijk_t = -20 ] ; then TRACOEF2D=1 ; fi
     if [ $nn_aht_ijk_t = -30 ] ; then TRACOEF3D=1 ; fi
   fi

DYNCOEF2D=0  ; DYNCOEF3D=0
   tmp=$(LookInNamelist ln_dynldf_OFF namelist) ; tmp=$(normalize $tmp)
   if [ $tmp = F ] ; then   #  Diffusion is ON !
     nn_ahm_ijk_t=$(LookInNamelist nn_ahm_ijk_t namelist)
...
tmp=$(LookInNamelist ln_ensemble namelist) ; tmp=$(normalize $tmp)
if [ $tmp = T ] ; then  ENSEMBLE=1 ; fi

if [ $ENSEMBLE = 1 ] ; then
      # check if inter members diags are requested
    tmp=$(LookInNamelist ln_ens_diag  namelist) ; tmp=$(normalize $tmp)
    if [ $tmp = T ] ; then  ENSDIAGS=1 ; fi

      # set ensemble parameters from namelist
    ENSEMBLE_SIZE=$(LookInNamelist nn_ens_size  namelist)   
    ENSEMBLE_START=$(LookInNamelist nn_ens_start  namelist)
    ...
    tmp=$(LookInNamelist ln_ens_namlist  namelist) ; tmp=$(normalize $tmp)
    ...
    tmp=$(LookInNamelist ln_ens_rst_in  namelist) ; tmp=$(normalize $tmp)
    if [ $tmp = T ] ; then ENSEMBLE_RST_IN=1 ; fi
    
      # check the number of available cores vs the required number of cores
    jpnij=$(LookInNamelist jpnij)  # cor  nameliste per member
...
STO=0 ; RSTO=0
tmp=$(LookInNamelist ln_sto_ldf  namelist) ; tmp=$(normalize $tmp)
if [ $tmp = T ] ; then STO=1 ; fi
tmp=$(LookInNamelist ln_sto_hpg  namelist) ; tmp=$(normalize $tmp)
if [ $tmp = T ] ; then STO=1 ; fi
tmp=$(LookInNamelist ln_sto_eos  namelist) ; tmp=$(normalize $tmp)
if [ $tmp = T ] ; then STO=1 ; fi

if [ $STO = 1 ] ; then
    tmp=$(LookInNamelist ln_rststo  namelist) ; tmp=$(normalize $tmp)
...
eof
    ...
    ndate0=$(LookInNamelist nn_date0  namelist)
...
if [ $ELMERCPL = 1 ] ; then
    if [ $no = 1 ] ; then
       ...
       rapatrie $CN_DOMCFG $DDIR/${CN_DIRRST}.$((no-1)) NONE $CN_DOMCFG
    fi
else
    ## DOMAINcfg
    if [ $DOMAINcfg = 1 ] ; then  
...
if [ $no -eq  1 ] ; then
    ...
    tmp=$(LookInNamelist ln_ens_rst_in namelist ) ; ln_ens_rst_in=$( normalize $tmp )
        ...
        for member in $(seq $ZENSEMBLE_START  $ZENSEMBLE_END ) ; do
            ...
            if [ $RST_READY = 1 ] ; then
                OCE_RST_IN=$(LookInNamelist cn_ocerst_in   namelist)-${prev_ext}$mmm
                ...
                ICB_RST_IN=$(LookInNamelist cn_icbrst_in   namelist)-${prev_ext}$mmm
            else
##### O C E A N
###############
                OCE_RST_IN=$(LookInNamelist cn_ocerst_in   namelist)$mmm
                OCE_RST_OUT=$(LookInNamelist cn_ocerst_out   namelist)$mmm
                ok=1
...
###########
            if [ $RSTO = 1 ] ; then
                STO_RST_IN=$(LookInNamelist cn_storst_in  namelist)$mmm 
                STO_RST_OUT=$(LookInNamelist cn_storst_out  namelist)$mmm 
...
###########
            if [ $ICB = 1 ] ; then  # need iceberg restart
                ICB_RST_IN=$(LookInNamelist cn_icbrst_in    namelist)$mmm
                ICB_RST_OUT=$(LookInNamelist cn_icbrst_out   namelist)$mmm
...
cp layout.dat $P_S_DIR/ANNEX/
  ...
  tmp=$(LookInNamelist ln_timing   namelist) ; tmp=$( normalize $tmp )
  if [ $tmp = 'T' ] ; then ntiming=1 ; fi  
  tmp=$(LookInNamelist ln_meshmask   namelist) ; tmp=$( normalize $tmp )
...
case $STOP_FLAG in
    ...
    for member in  $(seq $ENSEMBLE_START $ENSEMBLE_END) ; do
       ...
       # O C E A N 
       # *********
        OCE_RST_IN=$(LookInNamelist cn_ocerst_in   namelist)
        OCE_RST_OUT=$(LookInNamelist cn_ocerst_out   namelist)
       ...
       # ************
        if [ $TRDMLD = 1 ] ; then
            TRD_RST_IN=$(LookInNamelist cn_trdrst_in   namelist)
            TRD_RST_OUT=$(LookInNamelist cn_trdrst_out   namelist)
       ...
       # *****
        if [ $STO = 1 ] ; then
            STO_RST_IN=$(LookInNamelist cn_storst_in  namelist)
            STO_RST_OUT=$(LookInNamelist cn_storst_out  namelist)
...
#             tar cf $P_S_DIR/OBS/${CONFIG_CASE}${mmm}_sla_fdbk.$ext.tar sla${mmm}_fdbk_*.nc
    ...
    tar cf tarfile.${CONFIG_CASE}_annex.$ext *run.stat*.$ext *ocean.outpu*.$ext *namelist.$ext
