...
#if defined key_tipaccs
      NAMELIST/namtra_eiv_tipaccs/ nn_ldfeiv_shape
#endif
      !!----------------------------------------------------------------------
      !
      IF(lwp) THEN                      ! control print
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtra_eiv in configuration namelist' )
      ...
      !
#if defined key_tipaccs
      READ  ( numnam_ref, namtra_eiv_tipaccs, IOSTAT = ios, ERR = 903)
903   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namtra_eiv in reference namelist' )
      !
      READ  ( numnam_cfg, namtra_eiv_tipaccs, IOSTAT = ios, ERR = 904 )
904   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtra_eiv in configuration namelist' )
      IF(lwm)  WRITE ( numond, namtra_eiv_tipaccs )
#endif
      !
      IF(lwp) THEN                      ! control print
...
#if defined key_tipaccs
      REAL(wp) :: z2_3
      REAL(wp), DIMENSION(jpi,jpj) :: zratio
#endif
      !!----------------------------------------------------------------------
      !
